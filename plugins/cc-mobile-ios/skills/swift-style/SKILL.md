...
**Domain-facing signatures return `Outcome<Success>`.** The two types are defined once in `AppCore` (`Outcome.swift`, `DomainError.swift` — see `${CLAUDE_PLUGIN_ROOT}/skills/ios-app-skeleton/references/app-core.md`); every use case, repository method, and view model that needs a typed failure goes through them.
