...
- Hard rules (no `kapt` — KSP only; no string routes; no `android.` imports in `:core:domain`; Compose BOM — never pin Compose libs individually).
...
4. Write root `build.gradle.kts` and `settings.gradle.kts`.
5. Write module `build.gradle.kts` for `:app`, `:core:domain`, `:core:data`.
6. Write `AndroidManifest.xml`, `strings.xml`, themes, one launcher icon set (mipmap-anydpi-v26 vector placeholder).
7. Write source files:
   - `:app` — `MyApplication.kt` (Hilt), `MainActivity.kt`, `ui/theme/` (Material 3), `ui/splash/SplashScreen.kt` + `SplashViewModel.kt`, `navigation/AppNavGraph.kt` (typed `@Serializable` destinations).
   - `:core:domain` — `Outcome.kt` sealed, `DomainError.kt`, one sample use case interface.
   - `:core:data` — `network/ApiClientFactory.kt` (Retrofit + OkHttp logging), `network/RemoteDataSource.kt` placeholder, DI module.
   - (Conditional) `:core:data/persistence/` with Room DB, entities, DAOs, repository.
   - (Conditional) `:core:data/datastore/` with DataStore typed preferences.
   - (Conditional) `:app` — Firebase init in `MyApplication`, gitignored `google-services.json` reminder.
8. Write one ViewModel test (`SplashViewModelTest.kt`) using JUnit 4 + MockK + Turbine.
...
11. Print the manual setup note — signing config (`signingConfigs.release`), per-flavor `applicationIdSuffix`, CI, Play Console metadata.
