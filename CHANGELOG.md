...
- **`cc-mobile-ios` gains `tests/triggers.json` and `.mcp.json`**, closing the parity gap with `cc-mobile-android`. 18 tier-1 routing tests (one positive per model-invocable skill, plus three anti-triggers: off-topic, an Android/Compose cross-platform prompt, and a generic DI explainer), validated against `schemas/triggers.schema.json`. Prompts are written to discriminate between the pairs that overlap — `keychain-secure-storage` vs `ios-security`, `swift-style` vs `swift-concurrency`, `swiftui-views` vs `navigation-stack`, `ios-architecture` vs `ios-di`.
- **Figma MCP wired into the iOS UI path.** `.mcp.json` declares Figma's official server (OAuth, no API key); `swiftui-views` and `ios-ui-engineer` gained a "pulling design specs from Figma" section that maps Figma tokens onto asset-catalog colours and semantic `Font` text styles rather than raw hex/pt — literals ignore dark mode and break at large Dynamic Type sizes. The plugin README documents the OAuth-on-first-use behaviour; `cc-mobile-android` ships the same server but doesn't yet document it.
